...
func InsertVersions() (err error) {
	...
	//versionIdScriptEC2100, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 1, 31, 13, 46, 23, 0, time.Local), 1, 0, 0, 0, "Script created")
	//versionIdScriptEC2101, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 2, 21, 16, 23, 40, 0, time.Local), 1, 0, 1, 0, "Fixed broken call")
	//versionIdScriptEC2102, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 3, 02, 16, 02, 13, 0, time.Local), 1, 0, 2, 0, "Added error checking")
	//versionIdScriptEC2110, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 6, 17, 9, 12, 03, 0, time.Local), 1, 1, 0, 0, "Added support for selecting a region")
	//versionIdScriptEC2120, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 8, 24, 9, 22, 32, 0, time.Local), 1, 2, 0, 0, "Added support for selecting a machine size")
	//versionIdScriptEC2121, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 8, 25, 06, 34, 54, 0, time.Local), 1, 2, 1, 0, "Corrected call to AWS")
	//versionIdScriptEC2122, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 8, 27, 11, 43, 12, 0, time.Local), 1, 2, 2, 0, "Added error checking on the machine size")
	//versionIdScriptEC2200, err = version.GormInsertVersion(scriptIdEC2, time.Date(2021, 9, 9, 9, 12, 9, 0, time.Local), 2, 0, 0, 0, "Added operating system, disk size support")
